...
async function getTrousersDataWomen() {
  try {
    const trousersData = await Trousers.find();
    const womenTrousers = trousersData.filter(item => item.trousers_sex === "women")
    return womenTrousers;
  } catch (error) {
    console.error("Error fetching trousers data:", error);
    return [];
  }
}
module.exports = { passOnTrousersWomen, getTrousersDataWomen };